    ...
    <div className="min-h-screen bg-slate-50">
      <section className="border-b bg-white py-16 text-center">
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-green-700 mb-3">Pricing</p>
        ...
        <div className="grid gap-6 md:grid-cols-3 items-start">
          {plans.map((plan) => (
            <div key={plan.name} className={`rounded-xl border p-6 md:p-8 ${
              plan.highlighted
                ? "border-green-500 bg-white shadow-lg ring-1 ring-green-500 scale-105"
              ...
              {plan.name === "Early Adopter" && (
                <p className="text-xs font-semibold uppercase tracking-wider text-green-600 mb-4">
                ...
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm text-slate-600">
                    <CheckCircle className={`h-4 w-4 mt-0.5 shrink-0 ${
                      plan.highlighted ? "text-green-500" : "text-slate-400"
                    }`} />
                    {f}
                  </li>
                ))}
